...
function Outcome({
  ...
  const verdict =
    tracked.status === 'won'
      ? { text: 'Selection hit', tone: 'text-status-good', mark: '✓' }
      : tracked.status === 'lost'
        ? { text: 'Selection missed', tone: 'text-ink-subtle', mark: '✗' }
        : tracked.status === 'push'
      ...
      <dl className="space-y-1.5 text-2xs">
        ...
        {typeof tracked.actual?.position === 'number' ? (
          ...
          selection.projection && (
            <div className="flex justify-between gap-3">
              <dt className="text-ink-faint">{settled ? 'Final score' : 'Current score'}</dt>
        ...
        <div className="flex justify-between gap-3">
          <dt className="shrink-0 text-ink-faint">Required</dt>
          <dd className="text-right text-ink-muted">{selection.explanation}</dd>
        </div>
      </dl>

      {verdict && (
        <p className={`mt-2 flex items-center gap-1.5 text-2xs font-medium ${verdict.tone}`}>
  ...
  return (
    <article className="panel p-4">
      ...
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-2xs">
        <span className="grid size-5 shrink-0 place-items-center rounded-md bg-violet-500/15 text-2xs font-medium text-violet-300">
          {index + 1}
        </span>
        {/*
        ...
        */}
        ...
        {selection.league && selection.league !== sportLabel(selection.sport) && (
          <>
            ...
            <span className="truncate text-ink-subtle">{selection.league}</span>
          </>
        )}
        <span className="text-ink-faint">·</span>
        <span className="truncate text-ink-subtle">{kickoff(selection.start_time)}</span>
        {tracked && (
      ...
      <a
        ...
        className="mt-2 block truncate py-1 text-sm text-ink-muted transition hover:text-ink-strong focus-ring"
      >
        {selection.fixture}
      </a>

      {/* The bet — the largest thing on the card, as it should be */}
      <div className="mt-3 flex flex-wrap items-start justify-between gap-x-4 gap-y-2 border-t border-line pt-3">
        <div className="min-w-0 flex-1">
          <p className="text-lg font-semibold leading-tight text-ink-strong">{selection.label}</p>
      ...
      <div className="mt-3 rounded-xl border border-line bg-surface-1 px-3 py-2.5">
        <p className="text-2xs uppercase tracking-wider text-ink-faint">
          What needs to happen
        </p>
        <p className="mt-1 text-xs leading-5 text-ink">{selection.explanation}</p>
      </div>
      ...
      <Expandable label="Analysis">
        <div className="mt-3 space-y-3">
                ...
                {race.entrants.slice(0, 5).map((entrant, place) => (
                  ...
                  >
                    <span className="min-w-0 truncate text-ink-muted">
                      <span className="mr-2 tabular-nums text-ink-faint">{place + 1}</span>
                      {entrant.driver}
                    </span>
          ...
          <div className="border-t border-line pt-3">
            ...
            <p className="mt-1 text-2xs leading-5 text-ink-subtle">
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-line pt-3">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-line bg-surface-1 px-2 py-0.5 text-2xs text-ink-subtle">
