...
/**
 * One fixture a line could be built from, for a reader choosing between them.
 *
 * Summarised rather than listed in full: the picker needs to know a fixture
 * exists, what the model's best idea about it is and how much it trusts it —
 * not every market on it. Choosing a fixture is choosing a leg the optimiser
 * will pick; which market that turns out to be is the model's job.
 */
export interface FixtureOption {
  ...
  /** How many candidates this risk profile accepts on the fixture. */
  candidates: number;
  /** The one the optimiser would take, which is what picking this gets you. */
  best: Selection;
}

/**
 * The fixtures a line could be built from, strongest first.
 *
 * Ordered by the same score the optimiser ranks on, so the order of the list
 * is the order it would choose in — a reader working down from the top is
 * agreeing with the model rather than guessing at it.
 */
export function fixtureOptions(
  ...
  const byGame = new Map<string, Selection[]>();
  for (const selection of qualified) {
    const list = byGame.get(selection.game_id);
    if (list) list.push(selection);
    else byGame.set(selection.game_id, [selection]);
  }
  ...
  return options.sort((a, b) => b.best.score - a.best.score);
}
